  ...
  const handleNextButton = async () => {
    try {
      ...
      if (response?.data?.document !== undefined) {
        ...
        const addressArray = concatenatedFields.address_vehicle_owner
          .split(/\s+|\.|,/)
          .filter(Boolean);

        const rearrangedAddress = rearrangeAddress(addressArray);
        concatenatedFields.address_vehicle_owner = rearrangedAddress;

        // Use the rearranged address to update the state
        setData({
          ...data,
          ...concatenatedFields,
          address_vehicle_owner: rearrangedAddress,
        });
